...
async function makeSticker(input, text = '') {
  ...
  try {
    ...
    if (text) {
      const font = findFont()
      if (!font) throw new Error('Font sistem tidak ditemukan. Fitur teks di stiker membutuhkan font (biasanya ada di Termux/Android).')
...
async function makeTextSticker(text) {
  ...
  const font = findFont()
  if (!font) throw new Error('Font sistem tidak ditemukan. Fitur stiker teks membutuhkan font (biasanya ada di Termux/Android).')
