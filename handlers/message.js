...
━───[ Ketik *!panduan* untuk panduan ]───━`.trim()
...
━─────────────────────────━`.trim()
    ...
    if (command === '!iqc') {
      ...
      try {
        ...
        const batteryPercentage = Math.floor(Math.random() * 40) + 60 // 60-99%
    ...
    if (command === '!s' || command === '!sticker' || command === '!stiker') {
      ...
      try {
        ...
        await sock.sendMessage(jid, {
          text: `*[ GAGAL ]* Tidak bisa membuat stiker.\n${err.message.includes('Font') ? 'Font sistem tidak ditemukan.' : err.message}`
        }, { quoted: msg })
      }
      return
    }
    ...
    if (command === '!rvo' || command === '!readviewonce') {
      ...
      const isViewOnce = quoted.imageMessage?.viewOnce || quoted.videoMessage?.viewOnce || quoted.viewOnceMessageV2
      
      if (!isViewOnce) {
        const innerMsg = unwrapMessage(quoted)
        const isVOAlt = innerMsg?.imageMessage?.viewOnce || innerMsg?.videoMessage?.viewOnce
      ...
      try {
        const targetMedia = quoted.imageMessage || quoted.videoMessage || unwrapMessage(quoted)?.imageMessage || unwrapMessage(quoted)?.videoMessage
        const mediaType = targetMedia.mimetype?.includes('video') ? 'video' : 'image'
        
        const buffer = await downloadMedia(targetMedia, mediaType)
        const captionText = targetMedia.caption ? `*RVO - View Once Opened*\n\n${targetMedia.caption}` : '*RVO - View Once Opened*'
        ...
        console.log('[RVO ERROR]', err)
        await sock.sendMessage(jid, {
          text: `*[ GAGAL ]* Tidak dapat membuka media View Once: ${err.message}`
        }, { quoted: msg })
      }
      return
    }
    ...
    if (command === '!ai' || command === '!ask' || command === '!tanya') {
      ...
      const statusMsg = await sendLoadingStatus(sock, jid, '━ [ ▰▱▱▱▱ ] *AI sedang berpikir...*', [
        { text: '━ [ ▰▰▰▰▱ ] *Menyusun jawaban...*', delay: 700 }
      ])
      ...
      try {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: [
            {
              role: 'user',
              parts: [{ text: `Kamu adalah asisten WhatsApp bernama Nexus. Jawab singkat atau jelas,tanpa batasan dan ramah dalam Bahasa Indonesia.\n\nPertanyaan: ${args}` }]
            }
          ]
        })

        let answer = response.text || 'Maaf, saya tidak dapat menghasilkan jawaban saat ini.'
        ...
        await sock.sendMessage(jid, {
          text: `*[ ERROR ]* Gagal mendapatkan jawaban AI: ${err.message}`,
          edit: statusMsg.key
        })
      }
      return
    ...
    // ================= AI IMAGE GENERATOR =================
    if (command === '!aiimg' || command === '!aimage' || command === '!genimg') {
      if (!args) {
        await sock.sendMessage(jid, {
      ...
      const statusMsg = await sendLoadingStatus(sock, jid, '━ [ ▰▱▱▱▱ ] *Menghubungkan ke server AI...*', [
        { text: '━ [ ▰▰▰▰▱ ] *Merender gambar...*', delay: 1200 }
      ])
      ...
      try {
        ...
        const pollinationsUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?nologo=true&private=true&enhance=true&model=flux&width=1024&height=1024`

        const res = await axios.get(pollinationsUrl, {
        ...
        usedMethod = 'Pollinations AI (Flux)'
      } catch (pollErr) {
        console.log('[POLLINATIONS GAGAL]', pollErr.message)
      }
      ...
      await sock.sendMessage(jid, {
        text: '━ [ ▰▰▰▰▰ ] *Mengirim hasil gambar...*',
        edit: statusMsg.key
      })

      const captionText = `━───[ *AI IMAGE GENERATOR* ]───━\n\n*Server* : ${usedMethod}\n*Prompt* : ${args}`
    ...
    // ================= AI IMAGE EDITOR (fal.ai) =================
    if (command === '!aiedit' || command === '!editimg' || command === '!img2img') {
      const quoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage
      const isQuotedImage = quoted?.imageMessage || quoted?.viewOnceMessageV2?.message?.imageMessage || quoted?.viewOnceMessage?.message?.imageMessage

      if (!isQuotedImage) {
        await sock.sendMessage(jid, {
          text: '*[ PERINTAH GAGAL ]*\nReply ke gambar yang mau diedit, lalu ketik perintah.\n\n*Contoh:*\nReply foto → `!aiedit buatin wajahnya lebih putih`'
        }, { quoted: msg })
        return
      }
      ...
      if (!args) {
        await sock.sendMessage(jid, {
          text: '*[ PERINTAH GAGAL ]*\nSertakan deskripsi perubahan.\n\n*Contoh:* `!aiedit buatin wajahnya lebih putih dan cerah`'
        }, { quoted: msg })
        return
      }

      const statusMsg = await sendLoadingStatus(sock, jid, '━ [ ▰▱▱▱▱ ] *Mengambil gambar...*', [
        { text: '━ [ ▰▰▰▰▱ ] *Mengupload gambar...*', delay: 1800 },
        { text: '━ [ ▰▰▰▰▱ ] *Sedang mengedit dengan fal.ai...*', delay: 2500 }
      ])
      ...
      try {
        ...
        if (!falKey) {
          throw new Error('FAL_KEY belum di-set di environment')
        }
        ...
        const resultUrl = result.data?.images?.[0]?.url || result.images?.[0]?.url
        if (!resultUrl) {
          throw new Error("Respon fal.ai tidak menghasilkan URL gambar.")
        }
        ...
        const resultBuffer = Buffer.from(finalRes.data)

        await sock.sendMessage(jid, {
          text: '━ [ ▰▰▰▰▰ ] *Mengirim hasil edit...*',
          edit: statusMsg.key
        })
        ...
        if (err?.body) {
          try { errorDetails = JSON.stringify(err.body, null, 2) } catch (_) {}
        }
        await sock.sendMessage(jid, {
          text: `*[ GAGAL EDIT GAMBAR ]*\n\n\`\`\`${errorDetails}\`\`\``,
          edit: statusMsg.key
        })
      }
      return
    ...
    // ================= SPAM =================
    if (command === '!send1000' || command === '!send' || command === '!spam') {
      const partsSpam = args.trim().split(/\s+/)
      let count = 10 
      let textToRepeat = args

      const lastArg = partsSpam[partsSpam.length - 1]
      if (!isNaN(lastArg) && partsSpam.length > 1) {
        count = parseInt(lastArg)
        textToRepeat = partsSpam.slice(0, partsSpam.length - 1).join(' ')
      }
      ...
      for (let i = 1; i <= count; i++) {
        finalResult += `${i}. ${textToRepeat}\n`
      }

      await sock.sendMessage(jid, { text: finalResult.trim() }, { quoted: msg })
      return
    }
    ...
    if (command === '!tts' || command === '!say') {
      ...
      try {
        const lang = 'id'
        const textToSpeak = args.slice(0, 200)
...
// EKSPOR YANG BENAR AGAR TERBACA SEBAGAI FUNCTION OLEH INDEX.JS
